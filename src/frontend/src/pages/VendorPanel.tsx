...
function LoadingSkeleton() {
  return (
    <div className="space-y-3 p-4" data-ocid="vendor.loading_state">
      {[1, 2, 3].map((i) => (
...
export function VendorPanel() {
  ...
  const { data: customers = [], isLoading: loadingCustomers } =
    useGetCustomers("vendor");
  const { data: summaries = [] } = useGetSummariesBySource("vendor");
  const { mutate: saveCustomer, isPending: isSaving } = useSaveCustomer();

  const handleSave = () => {
    if (!customerName.trim()) {
      setNameError("Please enter a vendor name.");
      return;
    }
    ...
    if (exists) {
      setNameError("This vendor name already exists.");
      return;
    }
    setNameError("");
    saveCustomer(
      { name: customerName.trim(), source: "vendor" },
      { onSuccess: () => setCustomerName("") },
    );
  };

  return (
    <div className="flex h-full" data-ocid="vendor.panel">
      {/* Left panel */}
      <div className="w-80 shrink-0 border-r border-border bg-muted/20 flex flex-col">
        ...
        <div className="px-4 py-4 border-b border-border space-y-3">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              data-ocid="vendor.customer_name_input"
            />
            {nameError && (
              <p
                className="text-xs text-destructive font-body"
                data-ocid="vendor.name_field_error"
              >
                {nameError}
              </p>
            )}
          ...
          <Button
            ...
            data-ocid="vendor.save_customer_button"
          >
        ...
        {loadingCustomers ? (
          ...
          <div
            ...
            data-ocid="vendor.customer_list"
          >
            {customers.length === 0 ? (
              <div
              ...
              customers.map((customer, index) => {
                ...
                return (
                  <div
                    key={customer.id}
                    data-ocid={`vendor.customer.item.${index + 1}`}
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-5 px-8 text-center bg-background">
        ...
        <div className="space-y-2 max-w-sm">
          ...
          <p className="text-sm text-muted-foreground font-body leading-relaxed">
            Save vendor names here. Then use the{" "}
            <strong className="text-foreground">Upload</strong> panel to attach
            documents to a specific vendor.
          </p>
        </div>
      </div>
    </div>
